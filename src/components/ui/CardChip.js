...
const CardChip = ({elementType = 'div', headerLevel = 'h2', ...props}) => {
  ...
  return (
    <Wrapper className="flex h-32 w-[21.5rem] bg-white relative shrink-0 group">
      <div className="basis-32 shrink-0">
        <img src={`${cloudinaryUrl}${document_cloudinary_ids}`} alt={name} />
      </div>
      <div className="flex items-center grow">
        <div className="px-4 space-y-1">
          <div className="inline-flex gap-px px-1 py-px text-[.75rem] text-white bg-transparent-black rounded font-semibold items-center">
            <svg role="img" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-3 h-3">
